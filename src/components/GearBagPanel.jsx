  ...
  return (
    ...
    >
      <h2 className="text-4xl font-bold mb-4 flex items-center">
        {/* <img src={bagImg} alt="Backpack Icon" className="w-16 h-16 translate-y-[1px]"></img> */}
        <div
          className="relative w-56 h-96 bg-gradient-to-b from-green-700
         to-green-900 rounded-3xl border-[6px] border-green-950 shadow-2xl
          flex justify-center items-center cursor-pointer"
          // onClick={handleBackpackClick}
        >
          {/** Zipper track */}
          <div className="absolute top-8 left-1/2 w-1 h-72 bg-gray-300 rounded-full -translate-x-1/2 z-10"></div>

          {/** Zipper Pull */}
          <div className="absolute top-8 left-1/2 w-6 h-4 bg-yellow-400 rounded shadow-md -translate-x-1/2 z-10"></div>

          {/** Top Flap */}
          <div className="absolute top-0 w-full h-12 bg-green-950 rounded-t-3xl border-b-4 border-green-900 "></div>
      ...
      <p className="flex items-center text-slate-300 mb-6">
        There are the tools I pack for my climb.
      </p>
      <div className="grid grid-cols-2 gap-6">
        {skills.map((skill, index) => (
          <div
            key={index}
            className="bg-slate-800 p-4 rounded-xl text-center drop-shadow-md hover:scale-105 transition-transform"
          >
            <img
              src={skill.icon}
              alt={skill.name}
